  ...
  meta: {
    ...
    messages: {
      bearerCompareInRoute:
        "Bearer-shaped compare against process.env secret in api route handler violates Rule I. Use validateApiKey() with an ApiKey row. Suppress only with: // rello-platform-lint-disable-next-line: no-env-var-bearer-fallback -- <reason>",
      fallbackHeuristic:
        "Heuristic: api route handler references both validateApiKey() and process.env.{{name}} (suspected fallback path). Audit and confirm the env-var path is not a Bearer-fallback that bypasses the ApiKey row. Suppress only with: // rello-platform-lint-disable-next-line: no-env-var-bearer-fallback -- <reason>",
    },
  },
