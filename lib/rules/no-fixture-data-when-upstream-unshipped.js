...
module.exports = {
  meta: {
    ...
    messages: {
      mathRandom:
        "Heuristic: Math.random() in admin api aggregator route — Rule L candidate. If this is a calculator placeholder, return null and render placeholder instead.",
      improvisedZero:
        "Heuristic: hardcoded {{value}} adjacent to `{{commentText}}` — Rule L candidate. If this is a calculator placeholder, return null and render placeholder.",
      upstreamPrTodo:
        "Heuristic: TODO referencing an upstream PR (`{{commentText}}`) — Rule L candidate. Return null and render placeholder until upstream ships.",
      featureFlagPlaceholder:
        "Heuristic: feature-flag-toggled placeholder (process.env.{{name}}) — Rule L candidate. Return null and render placeholder until upstream ships.",
    },
  },
