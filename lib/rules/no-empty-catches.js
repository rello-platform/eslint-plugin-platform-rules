...
module.exports = {
  meta: {
    ...
    messages: {
      emptyCatchHandler:
        "Empty .catch() handler is forbidden (CLAUDE.md §Error handling). Log with operation name + entity ID. Suppress only with: // rello-platform-lint-disable-next-line: no-empty-catches -- <reason>",
      emptyCatchBlock:
        "Empty catch block is forbidden (CLAUDE.md §Error handling). Log with operation name + entity ID. Suppress only with: // rello-platform-lint-disable-next-line: no-empty-catches -- <reason>",
    },
  },
