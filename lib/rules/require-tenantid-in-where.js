...
module.exports = {
  ...
  create(context) {
     ...
     *      within the same enclosing function / arrow body / transaction block
     *      (covers tight cascade-delete clusters under one block marker).
     */
    function isMarkerSuppressed(node) {
      // Find the enclosing function/program scope to bound the block-marker
      // search (so a marker in one function doesn't suppress another).
      let blockStart = 0;
